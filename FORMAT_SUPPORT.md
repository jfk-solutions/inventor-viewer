...
| `.asm` | Solid Edge or Creo assembly | Siemens Solid Edge or PTC Creo, selected by native container signature | Yes | Closed | Current MIT-licensed `solidedge-file-format` CFB reader or `creo-file-format` PSB reader. Solid Edge saved occurrence transforms, references, child appearances, properties and diagnostics are resolved through the workspace API |
...
| `.prt` | NX part, assembly or drawing; or Creo part | Siemens NX / NX MCD or PTC Creo, depending on file origin | Yes | Closed | Native byte signatures route files to the current MIT-licensed `simaticnx-file-format` or `creo-file-format` reader. NX support includes modern SPLM and legacy CFB containers with embedded JT 9.x scene graphs, highest-detail LOD selection, placements, materials, vertex/native-face colors, base textures, metadata and previews; Creo `#UGC:2` PSB support includes persisted display strips, metadata, diagnostics and saved previews |
| `.psm` | Solid Edge sheet-metal part | Siemens Solid Edge | Yes | Closed | Current MIT-licensed `solidedge-file-format` reader for native CFB documents, saved display-cache tessellation, native styles, properties, saved views, feature names, selection provenance, model bounds and Parasolid payload metadata |
