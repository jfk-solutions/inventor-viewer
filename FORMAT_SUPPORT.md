...
| `.asm` | Solid Edge or Creo assembly | Siemens Solid Edge or PTC Creo, selected by native container signature | Yes | Closed | Current MIT-licensed `solidedge-file-format` CFB reader or `creo-file-format` PSB reader. Saved display meshes and diagnostics are exposed; Solid Edge occurrence/reference transforms are not yet decoded |
...
| `.par` | Solid Edge part | Siemens Solid Edge | Yes | Closed | Current MIT-licensed `solidedge-file-format` reader for native CFB documents, saved display-cache tessellation, model bounds, Parasolid payload metadata and diagnostics |
...
| `.prt` | NX part, assembly or drawing; or Creo part | Siemens NX / NX MCD or PTC Creo, depending on file origin | Yes | Closed | Native byte signatures route files to the current MIT-licensed `simaticnx-file-format` or `creo-file-format` reader. NX support includes modern SPLM and legacy CFB containers with embedded JT 9.x display geometry; Creo `#UGC:2` PSB support includes persisted display strips, metadata, diagnostics and saved previews |
| `.psm` | Solid Edge sheet-metal part | Siemens Solid Edge | Yes | Closed | Current MIT-licensed `solidedge-file-format` reader for native CFB documents, saved display-cache tessellation, model bounds, Parasolid payload metadata and diagnostics |
