...
| `.prt` | NX part, assembly or drawing; or Creo part | Siemens NX / NX MCD or PTC Creo, depending on file origin | Yes | Closed | The Creo reader's byte-level `isCreoFile` check claims native PSB content before routing; other PRT files remain available to `simaticnx-file-format`. NX support includes modern SPLM and legacy CFB containers with embedded JT 9.x scene graphs, highest-detail LOD selection, placements, materials, vertex/native-face colors, base textures, metadata and previews; Creo `#UGC:2` PSB support includes persisted display strips, metadata, diagnostics and saved previews |
...
| `.step`, `.stp` | STEP (ISO 10303) | Autodesk Fusion and most mechanical CAD systems | Yes | Open | Current bundled MIT-licensed `step-file-format` browser worker for dependency-free AP203/AP214/AP242 parsing, assemblies, colors and tessellation |
