...
| `.asm` | Solid Edge or Creo assembly | Siemens Solid Edge or PTC Creo, selected by native container signature | Yes | Closed | The current readers identify native content from bytes rather than trusting the shared extension. `solidedge-file-format` resolves Solid Edge occurrence transforms, references, saved configurations, child appearances, properties and diagnostics through its workspace API; `creo-file-format` handles Creo PSB assemblies |
...
| `.dft` | Solid Edge draft/drawing | Siemens Solid Edge | Yes | Closed | Current MIT-licensed `solidedge-file-format` reader exposes saved display-cache geometry where present and explicit diagnostics otherwise |
...
| `.par` | Solid Edge part | Siemens Solid Edge | Yes | Closed | Current MIT-licensed `solidedge-file-format` reader for native CFB documents, saved display-cache tessellation, native styles and face opacity, properties, saved views, feature descriptors, selection provenance, model bounds and Parasolid payload metadata |
...
| `.prt` | NX part, assembly or drawing; or Creo part | Siemens NX / NX MCD or PTC Creo, depending on file origin | Yes | Closed | The readers' byte-level `isNxFile` and `isCreoFile` checks validate native content before either claims the shared extension. NX support includes modern SPLM and legacy CFB containers with embedded JT 9.x scene graphs, highest-detail LOD selection, placements, materials, vertex/native-face colors, base textures, arrangements, metadata and previews; Creo `#UGC:2` PSB support includes persisted display strips, metadata, diagnostics and saved previews |
| `.psm` | Solid Edge sheet-metal part | Siemens Solid Edge | Yes | Closed | Current MIT-licensed `solidedge-file-format` reader for native CFB documents, saved display-cache tessellation, native styles and face opacity, sheet-metal parameters, PMI inventories, properties, saved views, feature descriptors, selection provenance, model bounds and Parasolid payload metadata |
...
| `.sh3d` | Sweet Home 3D project | Sweet Home 3D interior-design ecosystem | Yes | Open | Current format-triggered ZIP/XML scene reader for Sweet Home 3D 5.3+ projects; the official [format description](https://www.sweethome3d.com/blog/sweet-home-3d-5-3/) documents the `Home.xml` entry and [HomeXMLHandler DTD](https://www.sweethome3d.com/javadoc/com/eteks/sweethome3d/io/HomeXMLHandler.html) |
| `.sldasm`, `.sldprt`, `.slddrw` | SolidWorks assembly, part and drawing | Dassault Systèmes SolidWorks | Yes | Closed | Current MIT-licensed `solidworks-file-format` reader for guarded modern/OLE2 containers and ZIP workspaces, transformed assembly occurrences, multi-block display tessellation with normal recovery, feature-scoped materials, typed metadata, diagnostics and previews |
| `.step`, `.stp` | STEP (ISO 10303) | Autodesk Fusion and most mechanical CAD systems | Yes | Open | Current bundled MIT-licensed `step-file-format` streaming browser worker for Edition 3 companion references, ZIP/ZIP64 catalogs, AP203/AP214/AP242 assemblies, regularized Boolean solids, bounded half-spaces, analytic and locally refined spline curves/surfaces, materials, annotations, saved views and tessellation |
...
| `.xzip` | NX packaged document collection | Siemens NX | Yes | Closed | Current `simaticnx-file-format` ZIP/XZIP reader; exposes contained PRT documents and resolves sibling references |
...
| `.drw` | Creo drawing | PTC Creo Parametric | Yes | Closed | Current `creo-file-format` PSB reader exposes saved display content, preview, metadata and diagnostics where present |
...
| `.sec` | Creo Sketcher section | PTC Creo Parametric | Yes | Closed | Current `creo-file-format` PSB reader exposes saved display content, preview, metadata and diagnostics where present |
