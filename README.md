...
| `.prt` | PTC Creo parts with persisted display strips or saved-preview fallback, and Siemens NX parts, assemblies and drawings with embedded JT 9.x scene geometry, highest-detail LOD selection, transforms, colors, materials, textures and metadata; each native reader validates the file contents before claiming the shared extension |
| `.par`, `.psm` | Solid Edge parts and sheet-metal parts with native display-cache tessellation, styles and face opacity, properties, saved views, feature descriptors, model bounds and Parasolid payload metadata |
| `.asm` | Solid Edge or Creo assemblies; native CFB/PSB signatures select the reader, with Solid Edge occurrence transforms, references, saved configurations and native child appearances resolved from the workspace |
...
| `.step`, `.stp` | STEP parts, assemblies, regularized Boolean solids, bounded half-spaces, analytic and locally refined spline curves/surfaces streamed and tessellated locally through the bundled dependency-free `step-file-format` Worker protocol, including Edition 3 companion references, ZIP/ZIP64 catalogs, materials, annotations, saved views and occurrence hierarchy |
...
Creo support reads native Creo Parametric and legacy Pro/ENGINEER `#UGC:2` PSB files through the browser-first `creo-file-format` reader. Complete persisted display strips render as 3D meshes; when those are unavailable, the explicit saved preview is shown and parser diagnostics explain the missing analytic geometry. The reader's byte-level `isCreoFile` check claims native Creo content before routing, so shared `.prt` and `.asm` extensions are never treated as proof of format.

Solid Edge support reads native Compound File Binary `.par`, `.psm`, `.asm` and `.dft` documents through the browser-first `solidedge-file-format` reader. Ambiguous extensions are routed only after the library's byte-level `isSolidEdgeFile` check identifies native Solid Edge evidence. The current workspace API resolves assembly references, exact saved occurrence transforms and saved assembly configurations; applies native document, record-level and transparent face styles; and exposes properties, saved views, feature descriptors, sheet-metal parameters, PMI inventories, cache records, persistent IDs and parser diagnostics. Native triangle selections retain their display-record and persistent-ID provenance. Exact Parasolid B-Rep and DFT sheet graphics are not yet decoded; documents without a supported saved display mesh report that limitation instead of substituting invented geometry.

Siemens NX support reads modern SPLM and legacy Compound File Binary PRT containers and decodes embedded JT 9.x display scenes locally. The reader's byte-level `isNxFile` check validates the native NX part stream before the viewer routes a `.prt`, avoiding collisions with Creo and unrelated formats that use the same extension. The viewer selects the highest-detail Range LOD alternative, preserves repeated scene paths and geometric transforms, and applies JT vertex colors, materials, native face colors and browser-decodable base textures. Part attributes, physical properties, arrangements, previews and detailed diagnostics are exposed as metadata. Reference-only assemblies resolve against sibling PRT files; until native NX assembly occurrence transforms are decoded, those components use the reader's explicitly diagnosed surrogate layout. JT 10.x and exact Parasolid B-Rep are not currently supported.
...
- `solidedge-file-format` and its optional Three.js adapter for Solid Edge CFB parsing, transformed assembly previews, reference and configuration resolution, native styles and face opacity, saved display-cache meshes, selection provenance, document summaries and ZIP workspaces
- `simaticnx-file-format` for content-based NX file identification, PRT/SPLM/CFB parsing, JT scene graphs, LODs, placements, materials, colors, textures, arrangements, metadata and multi-file reference resolution
...
- `step-file-format` for streamed, dependency-free STEP parsing and tessellation through its transferable Worker protocol, including Edition 3 external companions, ZIP/ZIP64 catalogs, AP203/AP214/AP242 Boolean and procedural solids, analytic and locally refined spline curves/surfaces, materials, annotations, saved views and occurrence hierarchy
